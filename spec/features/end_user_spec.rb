...
RSpec.feature "EndUserコントローラ関連", type: :feature do
  ...
  feature "ログインした状態で" do
    ...
    feature "表示内容とリンクの確認" do
      ...
      scenario "他人の詳細ページの表示内容とリンク" do
        ...
        expect(page).to have_no_link "",href: edit_end_user_path(@user2)
        expect(page).to have_button "フォローする"
        expect(page).to have_link "",href: following_end_user_path(@user2)
        expect(page).to have_link "",href: followers_end_user_path(@user2)
        expect(page).to have_selector "img"
    ...
    feature "自分のプロフィールの更新" do
      ...
      scenario "userが更新されているか" do
        ...
        expect(EndUser.find(1).nickname).to be_truthy
      end
    ...
    feature "有効ではない内容のuserの更新" do
      ...
      scenario "リダイレクト先が正しいか" do
        expect(page).to have_current_path edit_end_user_path(@user1)
      end
